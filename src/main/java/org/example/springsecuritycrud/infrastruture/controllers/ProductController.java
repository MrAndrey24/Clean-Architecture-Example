...
import org.example.springsecuritycrud.infrastruture.dto.ProductDTO;
import org.example.springsecuritycrud.domain.entities.Product;
import org.example.springsecuritycrud.application.service.ProductService;
...
public class ProductController {
    ...
    public ResponseEntity<GenericResponse> createProduct(@RequestBody Product newProduct) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(new GenericResponse(new ProductDTO(newId)));
        }catch (Exception e){
